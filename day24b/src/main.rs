...
fn a_star(t: i32, start: XY, goal: XY, map: &GameMap) -> Option<i32> {
...
fn main() {
    ...
    let start = (0, -1);
    let goal = (map.w as i32 - 1, map.h as i32);

    let t = a_star(0, start, goal, &map).unwrap();
    let t = a_star(t, goal, start, &map).unwrap();
    let t = a_star(t, start, goal, &map).unwrap();
