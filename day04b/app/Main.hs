...
anyOverlap :: forall a. Ord a => Range a -> Range a -> Bool
anyOverlap (Range min1 max1) (Range min2 max2) = min2 <= min1 && max1 <= max2 || -- 2 1 1 2
                                                 min1 <= min2 && max2 <= max1 || -- 1 2 2 1
                                                 min2 <= max1 && max1 <= max2 || -- 1 2 1 2
                                                 min2 <= min1 && min1 <= max2    -- 2 1 2 1
...
main = do
    ...
    if (all isLeft parsed) 
        then print (length (filter (\(x, y) -> anyOverlap x y) (lefts parsed)))
