...
fn find_best_path(
    ...
    let mut pairs = Vec::<(&Vec<String>, &Vec<String>, u32)>::new();

    // Hacky and dirty solution so I can go to bed :3
    for i in 0..valid_paths.len() {
        for j in i + 1..valid_paths.len() {
            let (ref path1, pressure1) = valid_paths[i];
            let (ref path2, pressure2) = valid_paths[j];

            if path1.iter().any(|it| path2.contains(it) && *it != "AA") {
                continue;
            }

            pairs.push((path1, path2, pressure1 + pressure2));
        }
    }

    pairs.sort_by_key(|(_, _, pp)| *pp);
