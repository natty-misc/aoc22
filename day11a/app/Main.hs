...
procIter monkeys vals = foldl (\acc (monkeyPos, monkey) -> doThrowTurn monkey monkeyPos (inspect monkey $ snd (acc !! monkeyPos)) acc) vals (zip [0..] monkeys)
...
main = do
    ...
    print . product . take 2 . reverse . sort $ fst <$> foldl (\acc _ -> mapper acc) ((\x -> (0, x)) <$> vals) [1..20]