...
aStar :: M.Map XY [XY] -> XY -> XY -> Maybe [XY]
aStar edges start end = AS.aStar (getNeighbors edges) (const $ const 1) (heuristic end) (==end) start

main :: IO ()
main = do
    inputLines <- lines <$> getContents
    let (Just start, Just end, vmap) = (parseMap (0, 0) $ inputLines)
    print $ length <$> aStar (mkEdges $ vmap) start end